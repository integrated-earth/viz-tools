...
public:
  ...
  StructuredData(const Point<3, double>            &min,
                 const Point<3, double>            &max,
                 const std::array<unsigned int, 3> &num_values,
                 const int                         &num_components)
  {
    ...
    TableIndices<4> t_ind(num_values[0],
                          num_values[1],
                          num_values[2],
                          num_components);
    data.reinit(t_ind);
  ...
  location_to_index(const Point<3, double> &p)
  {
    std::array<unsigned int, 3> index;
    for (int i = 0; i < 3; ++i)
      {
        unsigned int temp_index = std::floor((p(i) - min[i]) / spacing[i]);
        if (((1.0 * temp_index * spacing[i] + min[i]) +
             (1.0 * (temp_index + 1) * spacing[i] + min[i])) /
              2 <=
            p(i))
          {
            ++temp_index;
  ...
  Point<3, double>
  index_to_location(const std::array<unsigned int, 3> &idx)
  {
  ...
  // sets values in data table. priority is reciprodcal of distance from the
  // point to the vertex which we are writing to
  void
  set_values(const std::array<unsigned int, 3> &idx,
             double                             distance,
  ...
  std::array<unsigned int, 3>
  approximate_extent(const Point<3, double> &position, double radius)
  {
    // implement later
            ...
            for (unsigned int ix = 0; ix < 2 * extent[0]; ++ix)
              {
                ...
                current_index[0] = std::max(0l,
                                            static_cast<long>(idx[0] + ix) -
                                              static_cast<long>(extent[0]));
                current_index[1] = std::max(0l,
                                            static_cast<long>(idx[1] + iy) -
                                              static_cast<long>(extent[1]));
                current_index[2] = std::max(0l,
                                            static_cast<long>(idx[2] + iz) -
                                              static_cast<long>(extent[2]));
                ...
                const double distance =
                  index_to_location(current_index).distance(p);
  ...
  void
  to_vtk(Table<4, double>               &t,
         const Point<3, double>          min,
         const Point<3, double>          max,
         const std::string               filename,
         std::vector<DataInterpretation> component_type,
         const std::vector<std::string>  names)
  {
    ...
    for (int i = 0; i < 3; ++i)
      {
        spacing.push_back((double)(max[i] - min[i]) /
                          (double)(table_dim[i] - 1));
      }
    ...
    file << "<VTKFile type=\"ImageData\" "
            ...
            "<ImageData WholeExtent=\"" +
              std::to_string(0) + " " + std::to_string(n1 - 1) +
              ""
              " " +
              std::to_string(0) + " " + std::to_string(n2 - 1) + " " +
              std::to_string(0) + " " + std::to_string(n3 - 1) +
              ""
